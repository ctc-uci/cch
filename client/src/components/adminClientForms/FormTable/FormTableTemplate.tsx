...
export const FormTableTemplate = ({type}: TableType) => {
  ...
  const [initialInterviews, setInitialInterviews] = useState<
  ...
  const onPressCSVButton = () => {
    const selectedInitialInterviews = initialInterviews.filter((interview) =>
      selectedRowIds.includes(interview.id)
    );

    const data = selectedInitialInterviews.map((interview) => ({
      "First Name": interview.name.split(" ")[0],
      "Last Name": interview.name.split(" ")[0],
      "Phone Number": interview.phoneNumber,
      "E-mail": interview.email,
    }));

    downloadCSV(headers, data, `initialScreeners.csv`);
  };
  ...
  //doesn't work yet -- DELETE keyword isn't working and I'm scared of using it
  const handleDelete = async () => {
    try {
      await Promise.all(
        selectedRowIds.map((row_id) => backend.delete(`/initialInterview/${row_id}`))
      );
      setInitialInterviews(
        initialInterviews.filter((interview) => !selectedRowIds.includes(interview.id))
      );
      setSelectedRowIds([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let initialInterviewsRequest;
        if (searchKey && filterQuery.length > 1) {
          initialInterviewsRequest = backend.get(`/initialInterview/search-filter?page=&filter=${encodeURIComponent(filterQuery.join(" "))}&search=${searchKey}`);
        } else if (searchKey) {
          initialInterviewsRequest = backend.get(`/initialInterview/search-filter?page=&filter=&search=${searchKey}`);
        } else if (filterQuery.length > 1) {
          initialInterviewsRequest = backend.get(`/initialInterview/search-filter?page=&filter=${encodeURIComponent(filterQuery.join(" "))}&search=`);
        } else {
          initialInterviewsRequest = backend.get("/initialInterview");
        }
  
        const [lastUpdatedResponse, initialInterviewResponse] = await Promise.all([lastUpdatedRequest, initialInterviewsRequest]);
        const date = new Date(lastUpdatedResponse.data[0]?.lastUpdatedAt);
        setLastUpdated(date.toLocaleString());
        setInitialInterviews(initialInterviewResponse.data);

        console.log(initialInterviewResponse.data);
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <FilterTemplate setFilterQuery={setFilterQuery} />
        <HStack
          width="55%"
          justifyContent="space-between"
        >
          <Text fontSize="12px">
            showing {initialInterviews.length} results on this page
          </Text>
          <HStack>
            <Button></Button>
            <Text fontSize="12px">
              page {} of {Math.ceil(initialInterviews.length / 20)}
            </Text>
            <Button></Button>
          </HStack>
          <HStack>
