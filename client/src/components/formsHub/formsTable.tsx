...
export const createHashedId = (id: number, title: FormItem["title"]) => {
...
export const FormTable = () => {
  ...
  const [clickedFormItem, setClickedFormItem] = useState<FormItem>();
  ...
  const handleSelectAllCheckboxClick = () => {
    if (selectedRowIds.length === 0) {
      setSelectedRowIds(
        items.map((item) => createHashedId(item.id, item.title))
      );
    } else {
      setSelectedRowIds([]);
    }
                    ...
                    >
                      <HoverCheckbox
                        ...
                        isSelected={selectedRowIds.includes(
                          createHashedId(item.id, item.title)
                        )}
