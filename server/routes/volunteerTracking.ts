...
volunteerRouter.get("/:event", async (req, res) => {
    try {
      ...
      const users = await db.query(`SELECT * FROM volunteers WHERE event_type = $1 ORDER BY id ASC`, 
        [event, 
        ]);
      res.status(200).json(keysToCamel(users));
    } catch (err) {
...
volunteerRouter.get("/total-hours", async (req, res) => {
    try {
      const data = await db.query(`SELECT SUM(hours) AS total_hours FROM volunteers`);
...
volunteerRouter.get("/total-volunteers", async (req, res) => {
    try {
      const data = await db.query(`SELECT COUNT(DISTINCT first_name, last_name, email) AS total_volunteers FROM volunteers`);
      res.status(200).json(keysToCamel(data[0]));
    } catch (err) {
      res.status(500).send(err.message);
    }
