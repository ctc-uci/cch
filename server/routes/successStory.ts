...
successRouter.post("/", async (req, res) => {
  try {
    ...
    if (matchedClientId) {
      console.log("Success Story - Matched client ID:", matchedClientId);
    } else {
      console.log("Success Story - No client match found for:", clientFields);
    }
