...
donationRouter.get("/date", async (req, res) => {
    try {
      // Query database
      const {date} = req.body;
      const data = await db.query(`SELECT * FROM food_donations WHERE date = $1`, [date]);
...
donationRouter.post("/", async (req, res) => {
  try {
    ...
    const data = await db.query(`INSERT INTO food_donations (id, date, weight, value, category) VALUES ($1, $2, $3, $4, $5) RETURNING id`,
